...
def _ulink(user_id: int, username: str) -> str:
    return f'<a href="tg://user?id={user_id}">{escape(str(username))}</a>'
...
async def log_channel_added(chat_id: int, title: str, username: str,
    ...
    uname = f"@{escape(username)}" if username else f"<code>{chat_id}</code>"
    count = _count_line(member_count)
    await _send(
        f"📢 <b>Channel Added</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"┌ 📛 <b>{escape(title)}</b>\n"
        f"├ 🔗 {escape(uname) if uname.startswith("@") else uname}\n"
        f"├ 🆔 <code>{chat_id}</code>\n"
        f"{count}"
